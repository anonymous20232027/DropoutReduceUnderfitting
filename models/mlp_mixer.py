...
class MlpMixer(nn.Module):
    ...
    def updateDropPath(self, drop_path_rate):
        ...
        for block in self.blocks:
            block.drop_path = DropPath(dpr[cur]) if drop_path_rate > 0. else nn.Identity()
            cur += 1
    ...
    def updateDropOut(self, drop_rate):
        self.drop_rate = drop_rate
        for block in self.blocks:
            block.mlp_tokens.drop = nn.Dropout(drop_rate)
            block.mlp_channels.drop = nn.Dropout(drop_rate)

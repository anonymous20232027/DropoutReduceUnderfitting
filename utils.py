...
def reg_scheduler(base_value, final_value, epochs, niter_per_ep, warmup_epochs=0, warmup_value=None, 
           mode='linear', warmup_mode='regular'):
    warmup_schedule = np.array([])
    warmup_iters = warmup_epochs * niter_per_ep
    if warmup_value is None:
        warmup_value = final_value
    if warmup_epochs > 0:
        print(f"Set warmup value to {warmup_mode} {warmup_value}")
        if warmup_mode == 'regular':
            warmup_schedule = np.array([warmup_value] * warmup_iters)
        elif warmup_mode == 'linear':
            warmup_schedule = np.linspace(warmup_value, base_value, warmup_iters)
        elif warmup_mode == 'cosine':
            warmup_schedule = np.array(
            [base_value + 0.5 * (warmup_value - base_value) * (1 + math.cos(math.pi * i / warmup_iters)) for i in np.arange(warmup_iters)])
    regular_epochs = epochs - warmup_epochs
    ...
    elif mode == 'linear_epoch':
        schedule = np.array(
            [([i] * niter_per_ep) for i in np.linspace(base_value, final_value, regular_epochs)]).flatten()
    elif mode == 'period':
        schedule = np.tile(np.linspace(base_value, final_value, len(iters)//6), 6)
